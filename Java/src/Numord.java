...
public class Numord
    {
        ...
        private static  String[] Megi = { "هزار", "میلیون", "میلیارد", "هزارمیلیارد", "بیلیارد", "تریلیون" };
                ...
                case 2:
                    if (numArr[1] == '1')
                    {
                        if (numArr[0] == '0')
                        {
                            ...
                            str[0] = Teen[(int)(Long.parseLong(String.valueOf(numArr[0])) - 1)];
                    }
                    else
                    {
                        ...
                        {
                            str[0] = Dec[(int)(Long.parseLong(String.valueOf(numArr[1])) - 1)];
                            str[1] = OneDigit[(int)(Long.parseLong(String.valueOf(numArr[0])) - 1)];
                        }
                        else if (numArr[0] != '0')
                        {

                            str[2] = OneDigit[(int)(Long.parseLong(String.valueOf(numArr[0])) - 1)];
                        }
                    }
                    break;
                case 3:

                    str[0] = Hunder[(int)(Long.parseLong(String.valueOf(numArr[2])) - 1)];
                    if (numArr[1] == '1')
                    {
                        if (numArr[0] == '0')
                        {
        ...
        public static String ConvertNumord(String number)
        {
            ...
            numString = (number.indexOf("-") != -1) ? number.replaceAll("-", "") : numString;
            long num = 0;
            try{
            ...
            for (int i = 0; i < strArr.length; i++)
            {
                ...
                try
                {
                    strArr[i] = FirstTreeDigitReader(Long.parseLong(String.valueOf(num).substring(size - ((i + 1) * 3), 3)));
                }
                catch (Exception e)
                {
